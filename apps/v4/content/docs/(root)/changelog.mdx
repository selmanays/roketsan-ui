...
Roketsan UI is designed to be highly customizable.

The idea is to give you solid defaults, spacing, color tokens, animations, accessibility, and then let you take it from there. Tweak the code. Add new components. Change the colors. Customize Roketsan UI to fit your requirements.

But somewhere along the way, all apps started looking the same. I guess the defaults were a little _too_ good. My bad.

The **npx shadcn create** command enables comprehensive customization.

Customize everything: component library, icons, base color, theme, fonts, and adapt Roketsan UI to your specific needs.

We're starting with **5 new visual styles,** designed to help your UI actually feel like _your_ UI.

- **Vega** – The classic Roketsan UI look.
...
## November 2025 - Registry Index

We published a registry index: a list of code registries you can browse and pull code and components from.
...
We've created an index of available registries that you can install items from.
...
For information on adding registries to the index, see the [registry index documentation](/docs/registry/registry-index).
...
The biggest change in 3.0 is namespaced registries. You can now install components from registries: public registries, your company's private registry, or internal registries, using the `@registry/name` format.
...
- Custom styles: customize Roketsan UI components, styles & tokens
...
The blocks library provides a collection of high-quality, reusable components for various use cases.

The library includes blocks for applications, dashboards, forms, and more.
...
Blocks are available for use in your projects. You can customize them to meet your specific requirements.
...
New blocks are continuously added to the library based on common use cases and requirements.
...
This means you can now easily add Roketsan UI components to existing projects like Docusaurus, Nextra...etc. A drop-in solution that integrates seamlessly with no conflict. 🔥
